...
import { contentHash, proposedFileChangeSchema } from "./types.js";
...
function normalizeOptions(options) {
    ...
    return {
        maxFilesPerStep: Number.isFinite(maxFilesPerStep) && maxFilesPerStep > 0
            ? Math.floor(maxFilesPerStep)
            : defaultOptions.maxFilesPerStep,
        maxTotalDiffBytes: Number.isFinite(maxTotalDiffBytes) && maxTotalDiffBytes > 0
            ? Math.floor(maxTotalDiffBytes)
            : defaultOptions.maxTotalDiffBytes,
        maxFileBytes: Number.isFinite(maxFileBytes) && maxFileBytes > 0 ? Math.floor(maxFileBytes) : defaultOptions.maxFileBytes,
...
function buildStepCommitMessage(meta) {
...
export class FileSession {
    ...
    static async create(input) {
        const baseCommitHash = input.baseCommitHash === undefined ? await readCurrentCommitHash(input.projectRoot) : input.baseCommitHash;
    ...
    getLastCommittedDiffs() {
        return [...this.lastCommittedDiffs];
    }
    beginStep(stepId, stepIndex) {
        if (this.currentStep) {
    ...
    async stageChange(change) {
        ...
        if (parsed.type === "update") {
            ...
            if (parsed.oldContentHash !== existing.contentHash) {
                throw new Error(`Optimistic lock failed for '${normalizedPath}'. Expected ${parsed.oldContentHash}, found ${existing.contentHash}.`);
            }
            if (typeof parsed.newContent !== "string") {
        ...
        if (parsed.type === "delete") {
            ...
            if (parsed.oldContentHash && parsed.oldContentHash !== existing.contentHash) {
                throw new Error(`Delete optimistic lock failed for '${normalizedPath}'. Expected ${parsed.oldContentHash}, found ${existing.contentHash}.`);
            }
        }
    ...
    async applyStepChanges() {
        ...
        for (const staged of stagedChanges) {
            ...
            if (currentDisk.contentHash !== staged.previousContentHash) {
                throw new Error(`File lock check failed for '${staged.path}'. Expected ${staged.previousContentHash || "null"}, found ${currentDisk.contentHash || "null"}.`);
            }
        }
    ...
    async commitStep(meta) {
        const step = this.requireActiveStep();
        if (meta.stepId !== step.stepId || meta.stepIndex !== step.stepIndex) {
            throw new Error(`Commit metadata does not match active step. Active=${step.stepId}:${step.stepIndex}, Provided=${meta.stepId}:${meta.stepIndex}.`);
        }
        if (!step.applied) {
            await this.applyStepChanges();
        }
    ...
    async readFromDisk(absolutePath) {
    ...
    async restoreBackups(backups, stagedChanges) {
        for (const staged of stagedChanges) {
