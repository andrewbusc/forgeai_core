...
function hasBinaryLikeContent(value) {
    if (value.includes("\u0000")) {
        return true;
    }
    let suspicious = 0;
...
function hasUnsafeRelativePath(relativePath) {
    const normalized = relativePath.replaceAll("\\", "/");
    return normalized.startsWith("/") || normalized.includes("../") || normalized.includes("/..");
}
function isRestrictedEnvPath(relativePath) {
    const normalized = relativePath.replaceAll("\\", "/").toLowerCase();
    return normalized === ".env" || normalized.endsWith("/.env");
}
function isRestrictedByPrefix(relativePath, prefixes) {
    const normalized = relativePath.replaceAll("\\", "/");
