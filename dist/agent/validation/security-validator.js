...
function detectJwtSecretValidation(content) {
    if (!/AUTH_TOKEN_SECRET|JWT_SECRET|JWT_SECRET_KEY/.test(content)) {
        return false;
    }
    if (/if\s*\(\s*!\s*[A-Za-z0-9_]+\s*\)\s*\{[\s\S]{0,220}throw new Error\([^)]*required[^)]*\)/i.test(content) ||
        /(AUTH_TOKEN_SECRET|JWT_SECRET|JWT_SECRET_KEY)[\s\S]{0,220}required/i.test(content)) {
        return true;
    }
    return false;
}
export async function runSecurityBaselineValidation(projectRoot, contract = architectureContractV1) {
    ...
    const explicitEnvValidationDetected = byPath("src/config/env.ts").length > 0 ||
