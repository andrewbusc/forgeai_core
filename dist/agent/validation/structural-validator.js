...
function pushViolation(violations, input) {
    violations.push({
        ruleId: input.ruleId,
        severity: "error",
...
function hasNode20Constraint(raw) {
...
async function directoryContainsMatchingFile(directory, pattern) {
    let entries = [];
    try {
...
async function hasPrismaMigrationSql(projectRoot) {
    const migrationsDir = path.join(projectRoot, "prisma", "migrations");
    return directoryContainsMatchingFile(migrationsDir, /^migration\.sql$/i);
}
function parseDockerignoreEntries(content) {
    return content
        .split(/\r?\n/)
...
function hasDockerignoreTarget(entries, target) {
    if (target === "node_modules") {
        return entries.some((entry) => entry === "node_modules" ||
...
async function enforceDockerContract(root, violations) {
    ...
    if (await pathExists(dockerfilePath)) {
        ...
        try {
            ...
            pushViolation(violations, {
                ...
                file: "Dockerfile",
                message: `Could not read Dockerfile. ${String(error.message || error)}`
            });
        }
        if (dockerfile.length > 0) {
            const hasProductionNodeEnv = /\bENV\s+NODE_ENV\s*=\s*production\b/i.test(dockerfile) || /\bNODE_ENV\s*=\s*production\b/i.test(dockerfile);
            ...
            const hasProductionStartCommand = /CMD\s*\[\s*["']npm["']\s*,\s*["']run["']\s*,\s*["']start["']\s*\]/i.test(dockerfile) ||
    ...
    if (await pathExists(dockerignorePath)) {
        ...
        try {
            ...
            pushViolation(violations, {
                ruleId: "STRUCTURE.READ_ERROR",
                file: ".dockerignore",
                message: `Could not read .dockerignore. ${String(error.message || error)}`
            });
        }
        if (dockerignore.length > 0) {
            const entries = parseDockerignoreEntries(dockerignore);
            for (const target of ["node_modules", ".env", ".git"]) {
...
async function enforcePackageJsonContract(root, violations) {
    ...
    let parsed;
    try {
        parsed = JSON.parse(await readTextFile(packageJsonPath));
    }
    catch (error) {
        pushViolation(violations, {
            ruleId: "STRUCTURE.PACKAGE_JSON_INVALID",
            file: "package.json",
            message: `package.json must be valid JSON. ${String(error.message || error)}`
        });
        return;
    }
...
async function enforceModuleStructure(root, violations) {
    ...
    try {
        ...
        pushViolation(violations, {
            ruleId: "STRUCTURE.READ_ERROR",
            file: "src/modules",
            message: `Could not inspect modules directory. ${String(error.message || error)}`
        });
        return;
    }
    ...
    for (const moduleName of moduleNames) {
        ...
        for (const layer of requiredModuleLayers) {
            ...
            try {
                ...
                pushViolation(violations, {
                    ruleId: "STRUCTURE.READ_ERROR",
                    file: layerRelative,
                    message: `Could not inspect module layer '${layerRelative}'. ${String(error.message || error)}`
                });
                continue;
            }
            if (!hasLayerCode) {
        ...
        try {
            hasModuleTests = await directoryContainsMatchingFile(testsAbsolute, /\.(test|spec)\.(ts|tsx|js|jsx|mjs|cjs)$/i);
        }
        catch (error) {
            pushViolation(violations, {
                ruleId: "STRUCTURE.READ_ERROR",
                file: testsRelative,
                message: `Could not inspect test directory '${testsRelative}'. ${String(error.message || error)}`
            });
            continue;
        }
        if (!hasModuleTests) {
...
async function enforceTestAndMigrationArtifacts(root, violations) {
    ...
    if (await pathExists(integrationTestsDir)) {
        ...
        try {
            hasIntegrationTests = await directoryContainsMatchingFile(integrationTestsDir, /\.(test|spec)\.(ts|tsx|js|jsx|mjs|cjs)$/i);
        }
        catch (error) {
            pushViolation(violations, {
                ruleId: "STRUCTURE.READ_ERROR",
                file: "tests/integration",
                message: `Could not inspect integration tests directory. ${String(error.message || error)}`
            });
            return;
        }
    ...
    try {
        ...
        pushViolation(violations, {
            ruleId: "STRUCTURE.READ_ERROR",
            file: "prisma/migrations",
            message: `Could not inspect Prisma migrations. ${String(error.message || error)}`
        });
        return;
    }
    if (!hasMigrationSql) {
...
export async function runStructuralValidation(projectRoot) {
