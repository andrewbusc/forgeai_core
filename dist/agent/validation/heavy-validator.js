...
function buildScopedValidationDatabaseUrl(databaseUrl, scope) {
...
async function probeHealth(urlText, timeoutMs = 1_200) {
...
async function runBootCheck(input) {
    ...
    const detailSuffix = lastProbeStatus !== null
...
async function runProductionConfigCheck(projectRoot) {
    ...
    const hasNodeEnvProductionSupport = /NODE_ENV[\s\S]{0,320}production/i.test(envConfigRaw) || /production[\s\S]{0,320}NODE_ENV/i.test(envConfigRaw);
    ...
    const hasGuardedStackExposure = /if\s*\(\s*[^)]*NODE_ENV[^)]*production[^)]*\)\s*\{[\s\S]{0,320}\bstack\b[\s\S]{0,320}\}/i.test(errorHandlerRaw);
...
function resolveScript(scripts, names) {
...
export async function runHeavyProjectValidation(input) {
    return withIsolatedWorktree({
        ...
        const validationDatabaseUrl = buildScopedValidationDatabaseUrl(process.env.AGENT_HEAVY_DATABASE_URL || process.env.DATABASE_URL, path.basename(isolatedRoot));
        ...
        if (shouldInstallDeps) {
            ...
            if (!hasNodeModules) {
                ...
                if (!install.ok) {
                    ...
                    failures.push(...parseCommandFailures({
        ...
        if (typeof scripts.start === "string" && scripts.start.trim()) {
            ...
            if (boot.status === "fail") {
                blockingCount += 1;
                const bootLogs = boot.details && typeof boot.details === "object" && !Array.isArray(boot.details)
                    ? boot.details.logs
                    : undefined;
                failures.push(...parseCommandFailures({
                    sourceCheckId: "boot",
