...
test("controller prisma import is flagged", async () => {
    ...
    try {
        ...
        assert.equal(violations.some((entry) => entry.ruleId === "AST.CONTROLLER_NO_PRISMA_IMPORT"), true);
    }
    finally {
...
test("service request import is flagged", async () => {
    ...
    try {
        const violations = await runAstValidation(root);
        assert.equal(violations.some((entry) => entry.ruleId === "AST.SERVICE_NO_REQUEST_IMPORT"), true);
    }
    finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("raw Error throw is flagged", async () => {
    ...
    try {
        const violations = await runAstValidation(root);
        assert.equal(violations.some((entry) => entry.ruleId === "AST.NO_RAW_ERROR_THROW"), true);
    }
    finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("typed errors are allowed", async () => {
    ...
    try {
        const violations = await runAstValidation(root);
        assert.equal(violations.some((entry) => entry.ruleId === "AST.NO_RAW_ERROR_THROW"), false);
    }
    finally {
        await rm(root, { recursive: true, force: true });
    }
