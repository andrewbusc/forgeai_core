...
function detectDatabase(deps, fileSet) {
...
function detectAuth(deps, fileSet) {
...
function detectPayment(deps, fileSet) {
    if (deps.has("stripe")) {
        return "Stripe";
    }
...
async function readPackageJsonLite(projectRoot) {
...
async function detectKeyFiles(projectRoot, filePaths) {
    const candidates = [
        "package.json",
        "README.md",
        ".env.example",
...
function collectDependencyMatches(deps, keys) {
    return keys.filter((key) => deps.has(key));
}
export async function analyzeProjectForMemory(projectRoot) {
